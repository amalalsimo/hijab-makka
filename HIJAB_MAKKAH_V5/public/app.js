(() => {
  ...
  function productCardHtml(p) {
    ...
    return `
      <article class="product-card hover-product-card" data-product="${escapeHtml(p.id)}">
        <div class="product-image-wrap">
          ...
          <div class="card-hover-panel" aria-label="خيارات المنتج">
            ...
            ${p.has_color_options && colors.length ? `
              <div class="card-option-block card-color-block">
                ...
                <div class="card-color-list">
                  ${colors.map(c => `<button type="button" class="card-color-dot card-color-chip ${selection.color===c.label ? "active" : ""}" data-card-color="${escapeHtml(c.label)}" data-card-color-id="${escapeHtml(c.id || "")}" data-card-color-image="${escapeHtml(c.image || "")}" data-card-product="${escapeHtml(p.id)}" title="${escapeHtml(c.label)}" aria-label="${escapeHtml(c.label)}"><span style="background:${escapeHtml(colorHex(c.label,c.hex))}"></span><b>${escapeHtml(c.label)}</b></button>`).join("")}
                </div>
              </div>` : ""}
        ...
        <div class="product-body">
          ...
          <button class="card-details-link" type="button" data-open-product="${escapeHtml(p.id)}">عرض صفحة المنتج</button>
        </div>
      </article>`;
  }
  ...
  function bindEvents() {
    document.addEventListener("click", (e) => {
      ...
      if (cardSize) {
        ...
        const currentCard = cardSize.closest(".product-card");
        if (currentCard) currentCard.outerHTML = productCardHtml(product);
        return;
      }
