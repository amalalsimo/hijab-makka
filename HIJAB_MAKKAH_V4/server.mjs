...
async function getProducts(force = false) {
  ...
  productCache = {
    at: Date.now(),
    raw: dedup.map((p) => p.raw),
    safe: dedup.map((p) => p.safe).filter((p) => p.available !== false),
  };
  return productCache;
}
...
function selectedColor(rawProduct, item, size) {
  ...
  const sources = [
    ...(Array.isArray(variant?.colors) ? variant.colors : []),
...
async function createOrder(req, res) {
  ...
  for (const item of items.slice(0, 12)) {
    ...
    const quantity = Math.min(4, Math.max(1, Number(item.quantity || 1)));
  ...
  const parentOrderId = `HMC-${Date.now().toString(36).toUpperCase()}-${crypto.randomBytes(2).toString("hex").toUpperCase()}`;
  const results = [];
