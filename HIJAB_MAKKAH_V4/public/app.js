(() => {
      ...
      <article class="product-card" data-product="${escapeHtml(p.id)}">
        ...
        <div class="product-body">
          ...
          <div class="product-actions">
            <button class="view-btn" data-open-product="${escapeHtml(p.id)}"><i class="fa-solid fa-eye"></i> شوفي التفاصيل</button>
            <button class="quick-btn" data-open-product="${escapeHtml(p.id)}" aria-label="فتح"><i class="fa-solid fa-arrow-up-right-from-square"></i></button>
          </div>
        </div>
      </article>`).join("");
  }
  ...
  function applyRouteAfterProducts() {
    ...
    if (product) openProduct(product.id, { fromRoute: true });
  }
        ...
        <div class="page-product-info">
          ...
          <div class="page-order-box">
            ...
            <div class="page-actions">
              <button class="page-add-btn" data-page-add type="button"><i class="fa-solid fa-bag-shopping"></i> أضيفي للطلب</button>
  ...
  function openProduct(id, options = {}) {
    ...
    if (!options.fromRoute && location.pathname !== `/product/${encodeURIComponent(p.id)}`) {
      history.pushState({ productId: p.id }, "", `/product/${encodeURIComponent(p.id)}`);
    }
    ...
    const codeBox = $(".product-code");
    let direct = $("#productDirectLink");
    if (!direct) {
      direct = document.createElement("button");
      direct.type = "button";
      direct.id = "productDirectLink";
      direct.className = "product-direct-link";
      direct.innerHTML = '<i class="fa-solid fa-link"></i> نسخ رابط المنتج';
      const wrap = document.createElement("div");
      wrap.className = "product-code-link";
      codeBox.parentNode.insertBefore(wrap, codeBox.nextSibling);
      wrap.appendChild(direct);
      direct.addEventListener("click", async () => {
        try {
          await navigator.clipboard.writeText(productUrl(state.selected));
          toast("تنسخ رابط المنتج.");
        } catch {
          toast(productUrl(state.selected));
        }
      });
    }
